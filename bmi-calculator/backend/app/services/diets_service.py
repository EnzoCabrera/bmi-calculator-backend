...
def calculate_diet(db: Session, user_bmi, bmi_status_id: int, user_id: int, diet: Diet, intolerances: List[str]):
    ...
    prompt = (
        ...
        f"IMPORTANTE: Use apenas os pratos listados abaixo, sem criar variações, nomes diferentes ou sinônimos. "
        f"Você DEVE usar exatamente o mesmo nome do prato conforme listado, com a mesma ortografia e pontuação. "
        f"NÃO invente pratos, NÃO altere nomes e NÃO use traduções. Somente os nomes exatos da lista. "
